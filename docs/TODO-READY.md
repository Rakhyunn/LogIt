...
- [x] 홈 페이지 `+ 등록` 버튼 제거 (헤더에 통합)
- [x] 아바타 이미지 업로드 + 위치 조정 (avatars 버킷, drag focal point)
- [x] 로딩/에러 상태 처리 (스켈레톤 UI, 에러 페이지)
- [x] 전체 UI/UX 따뜻한 톤 리디자인 (크림 팔레트, Playfair Display, 카드 스타일)
- [ ] 반응형 레이아웃 (보류)
