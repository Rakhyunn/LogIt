...
export default async function ReviewSection({ contentId }: { contentId: string }) {
  ...
  const [{ data: reviewsRaw }, { data: { user } }] = await Promise.all([
    supabase
      .from('reviews')
  ...
  const safeReviews = (reviewsRaw ?? []) as (Review & { profiles: { username: string } | null })[]

  const reviewsWithUsername: ReviewWithUsername[] = safeReviews.map((r) => ({
    ...r,
    username: r.profiles?.username ?? '알 수 없음',
  }))

  const avgRating =
    reviewsWithUsername.length > 0
      ? (
          reviewsWithUsername.reduce((sum, r) => sum + r.rating, 0) / reviewsWithUsername.length
        ).toFixed(1)
      : null
